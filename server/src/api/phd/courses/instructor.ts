...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        const courseDetails = await db
            ...
            .from(phdCoursesNew)
            .where(eq(phdCoursesNew.instructor, instructor));

        // Group by courseType
        const groupedCourses: Record<string, typeof courseDetails> = {};

        courseDetails.forEach((course) => {
            const key = course.courseType;
        ...
        // If you want keys like course1, course2, etc.
        const finalJson: Record<string, typeof courseDetails> = {};
        let i = 1;
        for (const [, value] of Object.entries(groupedCourses)) {
            finalJson[`course${i}`] = value;
            i++;
        }
